...
**Tagline:** Compassionate Care that Feels Like Home
...
- **Forms:** use a simple serverless approach, such as Vercel + a form handler like Formspree, or a Next.js API route that emails via Resend/SendGrid
...
### About
- Founder/owner story (your mother-in-law's background: this is a strong E-E-A-T signal for Google and a good source for LLM citation)
...
- [ ] `LocalBusiness` (or more specific `MedicalBusiness`/`HomeHealthCareService`) JSON-LD schema in root layout, including name, phone, address, service area (Forsyth County), hours
